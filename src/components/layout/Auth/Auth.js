...
const Login = () => {
  ...
  const [message, setMessage] = useState("Get a new account");
  ...
  const onSubmitCreds = (e) => {
    ...
    axios
      ...
      .catch((error) => {
        ...
        if (err_status === "failed" && err_message === "account-not-exist") {
          setIsError(true);
  ...
  return (
    <Fragment>
      <h1 className="text-md text-slate-200 flex lg:mr-2 lg:text-6xl font-semibold mb-4">
        Login
      </h1>
      <form
